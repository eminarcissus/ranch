...
listen(Opts) ->
	...
	%% first argument.
	gen_udt:listen(0, ranch:filter_options(Opts4,
		[backlog, ip, port],
		[])).
...
accept(LSocket, Timeout) ->
	gen_udt:accept(LSocket, Timeout).
...
recv(Socket, Length, Timeout) ->
	gen_udt:recv(Socket, Length, Timeout).

-spec send(inet:socket(), iodata()) -> ok | {error, atom()}.
send(Socket, Packet) ->
	gen_udt:send(Socket, Packet).
...
setopts(Socket, Opts) ->
	gen_udt:setopts(Socket, Opts).

-spec controlling_process(inet:socket(), pid())
	-> ok | {error, closed | not_owner | atom()}.
controlling_process(Socket, Pid) ->
	gen_udt:controlling_process(Socket, Pid).

-spec peername(inet:socket())
	-> {ok, {inet:ip_address(), inet:port_number()}} | {error, atom()}.
peername(Socket) ->
	gen_udt:peername(Socket).

-spec sockname(inet:socket())
	-> {ok, {inet:ip_address(), inet:port_number()}} | {error, atom()}.
sockname(Socket) ->
	gen_udt:sockname(Socket).

-spec shutdown(inet:socket(), read | write | read_write)
	-> ok | {error, atom()}.
shutdown(Socket, How) ->
	gen_udt:shutdown(Socket, How).

-spec close(inet:socket()) -> ok.
close(Socket) ->
	gen_udt:close(Socket).
